...
class AICodeGenie:
    ...
    # Build features list (safe, no braces in the list items)
    features_list = "\n".join([f"- {feature}" for feature in (features or [])])

    # Use a single closed f-string for README content
...
## 🎯 Summary
This project was generated by the AICodeGenie tool. It contains a lightweight frontend (HTML/CSS/JS), configuration and example data to help you get started quickly.

## 🧩 Features
{features_list or "- (no features specified)"}
...
## 🛠️ How to run (static preview)
1. Open the `frontend/index.html` file in your browser for a static preview.
2. Files created:
   - `frontend/index.html`
   - `assets/css/style.css`
   - `assets/js/app.js`
   - `config/app.config.json` (if generated)
   - `data/sample_expenses.json` (if sample data included)
    ...
    with st.spinner(" AI is generating your intelligent application..."):
        ...
        try:
            ...
            if result["status"] == "success":
                ...
                st.markdown("---")
                st.subheader(" AI Application Preview")
